...
namespace lc {
	...
	inline double ggx_d(double cos_theta, double roughness) {
		...
		return term1 / glm::max(glm::pi<double>() * term2 * term2, eps);
	}
	...
	inline double Gp(const Vec3 &v, const Vec3 &h, const Vec3 &n, double roughness) {
		...
		double div = 1.0 + glm::sqrt(1.0 + glm::pow(roughness, 4) * (1.0 - VoH2) / glm::max(VoH2, eps));
		return X * 2.0f / glm::max(div, eps);
	}
